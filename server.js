...
async function handelMovie(req, res)  {
    let url = `https://api.themoviedb.org/3/movie/550?api_key=e338bfa50b9f5e55c7b9fba4b6173e6d`;
    let respons = await axios.get(url);
    let moiveData =respons.data;
    cleanedData= moiveData.data.map(item=>{
        return new ListMoive(item.original_title,item.overview)
    })
     res.status(200).json(cleanedData)
}
